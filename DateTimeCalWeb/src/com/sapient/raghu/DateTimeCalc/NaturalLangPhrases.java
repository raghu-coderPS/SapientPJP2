...
public class NaturalLangPhrases extends DateAndDateOperations{
	...
	String inTermsofWeeks(String date)
	{
		...
		LocalDateTimeConvert obj1= new LocalDateTimeConvert();
		LocalDate localgivenDate=obj1.convert(givenDate);
		int givenmonth=localgivenDate.getMonthValue();
		int diff1=curmonth-givenmonth;
		diff1=diff1*4;
		int givenyear=localgivenDate.getYear();
		int curyear=now.getYear();
		diff1=diff1+((curyear-givenyear)*52);
		int diff=thisweek-thatweek+diff1;
		if(Math.abs(diff)==0)
		{
			return "This week";
		}
	...
	String inTermsofMonths(String date)
	{
		...
		int givenyear=localgivendate.getYear();
		int curyear=now.getYear();
		diff=((curyear-givenyear)*12)+diff;
		if(Math.abs(diff)==0)
		{
			return "This month";
		}
